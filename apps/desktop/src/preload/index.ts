...
import {
  ...
  MemoryCenterEntriesSchema, MemoryResolutionSchema,
...
const api: AiLoverDesktopApi = {
  ...
  memory: {
    ...
    resolve: async (input) => { const value = MemoryResolutionSchema.parse(input);
      await ipcRenderer.invoke(IPC_CHANNELS.memoryResolve, value); },
  },
  visuals: {
    get: async () => {
