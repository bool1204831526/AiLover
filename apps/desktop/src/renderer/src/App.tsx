...
function ChatView({ character, bootstrapError, modelConfigured, sourceMessageId, onClearSource, onOpenSettings }: {
  ...
  useEffect(() => {
    ...
    setScene(storedScene || '用户的房间｜私人生活空间，角色刚来到这里｜可互动物品：床、书桌、电脑、台灯、衣柜、窗户、水杯');
...
function ModelSettings({ onSaved }: { onSaved(): void }): React.JSX.Element {
  ...
  return <><header className="conversation-header"><div><span className="eyebrow">应用设置</span>
    <h2>模型服务</h2></div></header><div className="settings-page"><section className="settings-section">
      ...
      <label className="toggle-setting"><span><strong>允许自主散步</strong>
        <small>空闲时偶尔在当前屏幕内缓慢移动；聊天和错误动作不会被打断。</small></span>
        <input type="checkbox" checked={companionSettings.desktopPetRoamingEnabled}
          onChange={(event) => { setCompanionSaved(false); setCompanionSettings({ ...companionSettings, desktopPetRoamingEnabled: event.target.checked }); }} /></label>
